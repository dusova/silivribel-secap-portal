...
if ($actionId <= 0) {
    ...
    header('Location: ' . BASE_PATH . '/mudurluk/eylemlerim');
    exit;
}
...
$stmt = $pdo->prepare('SELECT id, code, title FROM actions WHERE id = :id AND deleted_at IS NULL LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $maxSoStr = $pdo->prepare('SELECT MAX(sort_order) FROM activities WHERE action_id = :aid AND deleted_at IS NULL');
...
<div class="d-flex align-items-center gap-2 mb-4">
    <a href="<?= BASE_PATH ?>/mudurluk/eylemlerim" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i></a>
    <div>
    ...
    <div class="card-body">
        <form method="POST" novalidate>
            ...
            <div class="d-flex gap-2">
                ...
                <a href="<?= BASE_PATH ?>/mudurluk/eylemlerim" class="btn btn-outline-secondary">İptal</a>
            </div>
        </form>
    </div>
</div>
