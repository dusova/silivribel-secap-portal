...
foreach ($actionRows as $row) {
    ...
    $kpiStmt = $pdo->prepare(
        "SELECT k.id AS kpi_id, k.name AS kpi_name, k.unit, k.target_value,
                k.baseline_value, k.is_cumulative, k.activity_id, k.evidence_required,
                de.value AS current_value, de.year AS entry_year, de.is_verified,
                de.workflow_status, de.review_comment, de.id AS entry_id,
                (SELECT COUNT(*) FROM entry_attachments ea WHERE ea.data_entry_id = de.id AND ea.deleted_at IS NULL) AS attachment_count
         FROM   kpis k
         LEFT JOIN data_entries de ON de.kpi_id = k.id
            AND de.year = YEAR(NOW())
            AND de.department_id = :dept_id
            AND de.deleted_at IS NULL
         WHERE  k.action_id = :action_id
           AND  k.is_active = 1
           AND  k.deleted_at IS NULL
         ORDER  BY k.activity_id, k.id"
    );
    $kpiStmt->execute([':action_id' => $aid, ':dept_id' => $deptId]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-bold mb-0">Müdürlük Eylemleri</h5>
        <small class="text-muted"><?= count($actions) ?> eylem · <?= $currentYear ?> yılı veri durumu</small>
    </div>
    <a href="<?= BASE_PATH ?>/mudurluk/veri-girisi" class="btn btn-success">
        <i class="bi bi-plus-lg me-1"></i>Yeni Veri Kaydı
    </a>
</div>
...
<div class="card mb-3">
    ...
    <div class="px-3 py-2" style="background:var(--renk-ana-50); border-bottom:1px solid var(--renk-notr-200);">
        <div class="d-flex justify-content-between align-items-center">
            <small class="fw-semibold text-success"><i class="bi bi-layers me-1"></i>Faaliyetler</small>
            <a href="<?= BASE_PATH ?>/mudurluk/faaliyet-ekle?action_id=<?= $a['id'] ?>" class="btn btn-sm btn-outline-success py-0 px-2 yazi-2xs">
            ...
            <table class="table table-hover mb-0 yazi-orta">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="text-end"><?= $currentYear ?> Değeri</th>
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <?php if ($hasEntry): ?>
                                ...
                                <div class="text-muted yazi-xs">(%<?= round($progress,1) ?> hedefe ulaşıldı)</div>
                                <?php endif; ?>
                            <?php else: ?>
                        ...
                        <td class="text-center">
                            ...
                            <?php else: ?>
                                ...
                                <?php if (!empty($k['review_comment'])): ?>
                                <div class="small text-muted mt-1" title="<?= htmlspecialchars($k['review_comment'], ENT_QUOTES, 'UTF-8') ?>">
                                    <?= htmlspecialchars(mb_strimwidth($k['review_comment'], 0, 36, '…'), ENT_QUOTES, 'UTF-8') ?>
                                </div>
                                <?php endif; ?>
                            <?php endif; ?>
                        </td>
                        <td class="text-center">
                            <?php
                                $entryStatus = $k['workflow_status'] ?? ($k['is_verified'] ? 'approved' : 'submitted');
                                $canEdit = !$hasEntry || $entryStatus !== 'approved';
                            ?>
                            <?php if ($canEdit): ?>
                                <a href="<?= BASE_PATH ?>/mudurluk/veri-girisi?kpi_id=<?= $k['kpi_id'] ?>&year=<?= $currentYear ?>"
                            ...
                            <?php else: ?>
                                <span class="text-muted" title="Onaylanmış kayıt düzenlenemez"><i class="bi bi-lock"></i></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <?php else: ?>
    <div class="card-body text-center text-muted py-3 yazi-orta">
