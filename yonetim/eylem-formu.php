...
if ($isEdit) {
    $stmt = $pdo->prepare('SELECT * FROM actions WHERE id = :id AND deleted_at IS NULL LIMIT 1');
    $stmt->execute([':id' => $id]);
    $action = $stmt->fetch();
    if (!$action) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['soft_delete_id']) && !empty($_POST['soft_delete_entity'])) {
    Auth::requireSuperAdmin();
    Csrf::check();
    $entity = $_POST['soft_delete_entity'];
    if (!in_array($entity, ['activities', 'kpis', 'actions'], true)) {
        Flash::error('Geçersiz silme hedefi.');
        header('Location: ' . BASE_PATH . '/yonetim/eylem-formu?id=' . $id);
        exit;
    }
    $targetId = (int) $_POST['soft_delete_id'];
    $reason   = Validator::text($_POST['delete_reason'] ?? '', 500);
    try {
        $snapStmt = $pdo->prepare("SELECT * FROM `{$entity}` WHERE id = :id AND deleted_at IS NULL LIMIT 1");
        $snapStmt->execute([':id' => $targetId]);
        $snapshot = $snapStmt->fetch() ?: [];
        if (empty($snapshot)) {
            Flash::error('Kayıt bulunamadı veya zaten silinmiş.');
        } elseif (SoftDelete::delete($pdo, $entity, $targetId, $reason, $snapshot)) {
        ...
        error_log('[SECAP][SoftDelete][action_form] ' . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $dup = $pdo->prepare('SELECT id FROM actions WHERE code = :c AND id != :id AND deleted_at IS NULL LIMIT 1');
    ...
    if (empty($errors)) {
        try {
            ...
            $afterDeptIds = array_values(array_unique(array_merge(
            ...
            $beforeDeptIds = $isEdit ? array_map('intval', $auditBefore['contributor_depts'] ?? []) : [];
            $notifyDeptIds = $isEdit ? array_values(array_diff($afterDeptIds, $beforeDeptIds)) : $afterDeptIds;
            foreach ($notifyDeptIds as $deptIdForNotice) {
                NotificationService::notifyDepartment(
                    $pdo,
                    (int) $deptIdForNotice,
                    'action_assigned',
                    'Yeni eylem atandı',
                    "{$data['code']} kodlu \"{$data['title']}\" eylemi müdürlüğünüze atandı.",
                    BASE_PATH . '/mudurluk/eylemlerim',
                    'normal',
                    'actions',
                    $id,
                    'action_assigned:' . $id . ':dept' . (int) $deptIdForNotice
                );
            }
...
<div class="d-flex align-items-center gap-2 mb-4">
    <a href="<?= BASE_PATH ?>/yonetim/eylemler" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i></a>
...
<div class="col-12">
    <div class="card h-100">
        <div class="card-header"><i class="bi bi-pencil-square me-2 text-primary"></i>Temel Bilgiler</div>
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                ...
                <div class="col-md-6">
                    ...
                    <textarea name="performance_indicators" class="form-control" rows="3" placeholder="KPI özet metni..."><?= $f('performance_indicators') ?></textarea>
                </div>
                <div class="col-md-6">
                    ...
                    <div class="row g-1">
                        ...
                        <div class="col-md-6">
                            <div class="form-check">
                                ...
                                <label class="form-check-label yazi-kucuk-arti" for="cd<?= $d['id'] ?>">
                                    <?= htmlspecialchars($d['name'], ENT_QUOTES, 'UTF-8') ?>
                                </label>
                            </div>
                        </div>
...
<div class="mt-4 d-flex gap-2">
    ...
    <a href="<?= BASE_PATH ?>/yonetim/eylemler" class="btn btn-outline-secondary">İptal</a>
</div>
</form>
    ...
    <div class="col-12 col-lg-6">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="<?= BASE_PATH ?>/yonetim/faaliyet-formu?action_id=<?= $id ?>" class="btn btn-sm btn-outline-success">
                    <i class="bi bi-plus me-1"></i>Faaliyet Ekle
                </a>
            </div>
            <div class="card-body p-0">
                <?php
                $acts = $pdo->prepare("SELECT act.*, d.name AS dept_name FROM activities act JOIN departments d ON d.id = act.department_id WHERE act.action_id = :id AND act.deleted_at IS NULL ORDER BY act.sort_order");
                $acts->execute([':id' => $id]);
                $acts = $acts->fetchAll();
                ?>
                <?php if (empty($acts)): ?>
                <div class="text-center text-muted py-3 yazi-orta">Henüz faaliyet eklenmemiş.</div>
                <?php else: ?>
                <div class="list-group list-group-flush">
                    <?php foreach ($acts as $act): ?>
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <div class="fw-medium yazi-orta"><?= htmlspecialchars($act['title'], ENT_QUOTES, 'UTF-8') ?></div>
                            <small class="text-muted"><?= htmlspecialchars($act['dept_name'], ENT_QUOTES, 'UTF-8') ?></small>
                        </div>
                        <div class="d-flex gap-1">
                            <a href="<?= BASE_PATH ?>/yonetim/faaliyet-formu?id=<?= $act['id'] ?>" class="btn btn-sm btn-outline-secondary py-0 px-2">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <?php if (Auth::isSuperAdmin()): ?>
                            <button type="button" class="btn btn-sm btn-outline-danger py-0 px-2"
                                    data-bs-toggle="modal" data-bs-target="#softDeleteModal"
                                    data-sd-id="<?= (int) $act['id'] ?>"
                                    data-sd-entity="activities"
                                    data-sd-label="<?= htmlspecialchars($act['title'], ENT_QUOTES, 'UTF-8') ?>"
    ...
    <div class="col-12 col-lg-6">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="<?= BASE_PATH ?>/yonetim/kpi-formu?action_id=<?= $id ?>" class="btn btn-sm btn-outline-primary">
                    <i class="bi bi-plus me-1"></i>KPI Ekle
                </a>
            </div>
            <div class="card-body p-0">
                <?php
                $kpis = $pdo->prepare("SELECT * FROM kpis WHERE action_id = :id AND is_active = 1 AND deleted_at IS NULL ORDER BY id");
                $kpis->execute([':id' => $id]);
                $kpis = $kpis->fetchAll();
                ?>
                <?php if (empty($kpis)): ?>
                <div class="text-center text-muted py-3 yazi-orta">Henüz KPI tanımlanmamış.</div>
                <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-hover mb-0 yazi-orta">
                        <thead class="table-light"><tr><th>KPI</th><th>Birim</th><th>Hedef</th><th>Sözlük</th><th></th></tr></thead>
                        <tbody>
                        <?php foreach ($kpis as $k): ?>
                        <?php
                            $dictionaryMissing = empty($k['measurement_method']) || empty($k['data_source']) || empty($k['formula']);
                        ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($dictionaryMissing): ?>
                                <span class="badge bg-warning-subtle text-warning"><i class="bi bi-exclamation-triangle me-1"></i>Eksik</span>
                                <?php else: ?>
                                <span class="badge bg-success-subtle text-success"><i class="bi bi-check2 me-1"></i>Tamam</span>
                                <?php endif; ?>
                            </td>
                            <td class="text-end" style="white-space:nowrap;">
                                <a href="<?= BASE_PATH ?>/yonetim/kpi-formu?action_id=<?= $id ?>&id=<?= $k['id'] ?>"
                                ...
                                <button type="button" class="btn btn-sm btn-outline-danger py-0 px-2"
                                        ...
                                        data-sd-id="<?= (int) $k['id'] ?>"
                                        data-sd-entity="kpis"
                                        data-sd-label="<?= htmlspecialchars($k['name'], ENT_QUOTES, 'UTF-8') ?>"
                                        title="Çöp Kutusuna Taşı">
                                    <i class="bi bi-trash"></i>
                                </button>
                                <?php endif; ?>
...
<?php if ($isEdit && Auth::isSuperAdmin()):
    $sdFormAction = BASE_PATH . '/yonetim/eylem-formu?id=' . $id;
    $sdTitle      = 'Kaydı Çöp Kutusuna Taşı';
    $sdLabel      = 'Kayıt';
