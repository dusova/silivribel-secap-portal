...
if ($isEdit) {
    $stmt = $pdo->prepare('SELECT * FROM activities WHERE id = :id AND deleted_at IS NULL LIMIT 1');
    $stmt->execute([':id' => $id]);
    $activity = $stmt->fetch();
    if (!$activity) {
        ...
        try {
            ...
            if ($isEdit) {
                ...
                NotificationService::notifyDepartment(
                    ...
                    "{$action['code']} eylemine bağlı \"{$data['title']}\" faaliyeti müdürlüğünüze atandı.",
                    BASE_PATH . '/mudurluk/eylemlerim',
                    'normal',
                    'activities',
                    $newActivityId,
                    'activity_assigned:' . $newActivityId
                );
